...
from adam_hd_adam import AdamHD_lr_Adam
from sgd_hd_adam import SGDHD_lr_Adam

from adam_hd_nag import AdamHD_lr_Nag
...
def train(opt, log_func=None):
    ...
    if opt.method == 'sgd':
        ...
        optimizer = AdamHD_lr_Adam(model.parameters(), lr=opt.alpha_0, weight_decay=opt.weightDecay, hypergrad_lr=opt.beta)
        ...
        optimizer = AdamHD_lr_Nag(model.parameters(), lr=opt.alpha_0, weight_decay=opt.weightDecay, hypergrad_lr=opt.beta)
    else:
...
def main():
    try:
        ...
        parser.add_argument('--epochs', help='stop after this many epochs (0: disregard)', default=1, type=int)
